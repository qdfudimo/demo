<template>
    ...
    <el-input class="my-at-input" :size="type === 'textarea' ? 'small' : size" v-model="useless">
        <template slot="append">
            <el-popover v-bind="{ ...popoverProps }" trigger="manual" v-model="listVisible">
                <slot name="popover"></slot>
                <div slot="reference" class="popup-btn" :style="{ 'left': `${popupLeft}px`, 'top': `${popupTop}px` }">
                </div>
            </el-popover>

            <div ref="contentEdit" :placeholder="placeholder"
                :class="['content-edit', 'el-input__inner', `${type}-type`]" :contenteditable="true"
                @keydown="editHandle" :style="{
                    'width': `${maxWidth}px`,
